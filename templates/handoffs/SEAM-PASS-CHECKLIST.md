...
- [ ] **A `#[cfg(unix)]` (or otherwise platform-gated) test must not leave a `use` at module scope.** If the module's only items are platform-gated, the import compiles out on the other target and reds the **Windows cross-check clippy** (`--target x86_64-pc-windows-gnu -- -D warnings`) with `-D unused-imports`. Gate the whole module (`#[cfg(all(test, unix))]`) or move the `use` inside the gated fn. The Windows cross-clippy is a **separate CI step** run after native clippy — a native-clippy pass does not prove it, and an earlier lint failure **masks** it until cleared. Re-run the exact CI Windows command against the final committed HEAD; don't trust a reviewer's paste. _(cortina mycelium-hook-consent-and-timeout: Stage-2 false-green win-clippy paste, CI-caught)_
...
- [ ] **FTS5 relevance discarded by `IN (subquery)` + `ORDER BY <static col>`.** When an FTS table (`memories_fts`) is referenced only inside a `WHERE id IN (SELECT id FROM fts WHERE fts MATCH ?)` subquery, `bm25(fts)` is **not referenceable** in the outer `ORDER BY` — so relevance is silently thrown away and rows rank by whatever static column the outer query names (`weight`, `created_at`). Under `LIMIT` the wrong rows survive. Fix: `JOIN fts ON fts.id = m.id` (1:1 on a unique key — verify the FTS table is external-content with 1:1 triggers so the JOIN can't fan out) and `ORDER BY bm25(fts) ASC, …`. Qualify select columns with the alias once the JOIN makes bare names ambiguous. When you find one such method, **grep the whole file for sibling FTS methods** — they travel in packs (`search_fts`, `search_fts_scoped`, `search_fts_in_topic` all shared it; only `search_fts_scoped` was fixed in the first pass). _(hyphae fts-bm25-relevance-ordering `1969eed`; siblings tracked in fts-bm25-sibling-methods)_
...
## Code-intelligence root / backend resolution (rhizome)

- [ ] **A new or changed LSP-backed tool must resolve the file's root the same way as its siblings.** There are three divergent paths and only one is correct: trait-dispatched read tools (`find_references`/`get_symbols`/`get_diagnostics`/`get_definition`) hardcode `self.default_root` and **discard the dispatcher's computed `root`**; concrete `LspBackend` tools pass it verbatim with no per-file walk; only `rename`/`preview_rename` call `detect_workspace_root(file, lang, stop_at)`. The last is the correct pattern — a tool that hands tsserver the broad launch root for a file in a **nested project** (its own `tsconfig.json`/`Cargo.toml` below the launch root) gets an inferred single-file project → silent undercount. Confirm the tool routes through the per-file `resolve_root`/`detect_workspace_root` seam, not a raw `default_root`/`project_root`. _(3rd occurrence: index-pruning blind spot → `analyze_impact` treesitter divergence → Tier-1 root-discard; live-reproduced 2026-06-23: 1 ref for a 6-file symbol)_
- [ ] **Don't infer the LSP failure mode — reproduce it.** "tsserver lazy-loads files" was asserted as fact and was wrong; the cause was the root handed to the server. When a code-intelligence result disagrees with grep ground-truth, run a controlled repro before writing the mechanism into a finding.
- [ ] **A result's scope/risk label must come from the result, not the backend capability.** `scope:"project"` is set from `capabilities().cross_file_references` (always "project" for LSP), so a degenerate single-file LSP result is mislabeled cross-file. Verify a status/scope field reflects the actual data, not a static capability flag.
...
- [ ] **Bounded-subprocess timeout that claims to kill the child:** the `std::thread` + `mpsc::recv_timeout` + `wait_with_output` pattern usually cannot kill on timeout — the worker `take()`s the `Child` out of the shared `Arc<Mutex<Option<Child>>>` to call `wait_with_output` (which consumes `self`), so the timeout arm's `guard.take()` returns `None` and `.kill()` is dead code. The child is reparented to the OS on exit, not terminated. If a guaranteed kill matters, keep the `Child` (or its pid) reachable to the timeout arm: split the stdout/stderr pipes into the worker and leave the handle in the mutex, or kill by `child.id()`. Assert the kill with a stub that hangs **before writing stdout** (a post-write hang dies of `EPIPE` and hides the bug). _(cortina mycelium-hook-consent-and-timeout Stage 2 #3; shared with `inject_recall_for_tool`)_
