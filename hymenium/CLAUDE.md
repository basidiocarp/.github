...
├── decompose.rs      # Split large handoffs into focused child handoffs
...
├── dispatch.rs       # Create Canopy tasks, assign agents by tier
├── monitor.rs        # Progress monitoring and escalation
...
- **dispatch.rs**: the only module that writes to Canopy; creates tasks and assigns agents, never shares the Canopy database directly.
- **monitor.rs**: polls Canopy state, evaluates completeness gates, and escalates when workflows stall beyond the configured timeout.
