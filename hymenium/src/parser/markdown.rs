            ...
            match value.0.as_str() {
                ...
                "Allowed write scope" => {
                    metadata.allowed_write_scope =
                        split_scope(&value.1).into_iter().map(|s| s.to_string()).collect();
                }
                "Cross-repo edits" => {
    ...
    for (idx, line) in lines.iter().enumerate() {
        if line.starts_with("### Step ") && line.contains(':') {
            if let Some(step_num) = extract_step_number(line) {
                ...
                if let Ok(step) = parse_step(step_num, title, step_lines) {
                    steps.push(step);
                }
            }
        }
...
fn parse_step(number: u32, title: String, lines: &[&str]) -> Result<ParsedStep, ParseError> {
