...
Do not treat this page as a substitute for tool-local `doctor` or `status` commands. Prefer inspection commands when they exist.
...
When installed, `canopy` owns coordination-runtime state. It should not overload Hyphae session storage for task ownership or handoff state.
