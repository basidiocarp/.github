...
Everything runs on your machine. SQLite databases, fastembed for local embeddings, no cloud accounts needed for the core stack. Your code never leaves your network.

## What This Means in Practice

Memory data stays in `~/.local/share/hyphae/hyphae.db`. Embeddings compute locally using fastembed (384 dimensions). Cortina hooks capture errors and corrections without uploading anything. Cap's dashboard runs on localhost:3000 and reads the database directly. No telemetry. No analytics. No usage tracking.
...
| Editor lock-in | Works with any MCP editor (Claude Code, Cursor, Windsurf, Continue) | Cursor or API | Framework-specific | Cursor only |
...
Point Hyphae's embedder at Ollama for larger local models (Nomic Embed Text is 768 dims, runs locally, free). Or use OpenAI-compatible endpoints—the config accepts any HTTP embedder with an `embed` endpoint. You get better search quality without moving your data to the cloud.
...
Cap's dashboard runs on localhost but can be exposed to a network if you own the infrastructure. Run it behind a reverse proxy and access from another machine on your LAN. Hyphae stays local; Cap just reads it.
