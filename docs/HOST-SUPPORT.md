...
This is not the same as saying every client has its own dedicated host mode. Shared MCP coverage means Basidiocarp can register servers in the client config, not that every client gets its own hook, notify, or lifecycle flow.
