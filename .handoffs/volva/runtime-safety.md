...
- [ ] Double-wait eliminated; exactly one `wait()` per child
- [ ] `AuthTarget` match arms are explicit (no wildcard `unreachable!`)
- [ ] Zombie child: discard-then-zombie pattern replaced with WNOHANG retry or proper wait
- [ ] `try_wait` OS errors logged before treating as "still running"
- [ ] Auth callback header read bounded to ≤ 8 KB
- [ ] `home_dir()` failure returns an error instead of falling back to CWD
