...
- [ ] Content-Length capped at ≤ 100 MB; excess returns error
- [ ] `register_mcp_server` returns error on non-object JSON root
- [ ] TOCTOU `exists()` checks replaced with try-open/try-create
- [ ] Tilde expansion applied to env-var path override
- [ ] Config backup uses write-to-temp-then-rename
- [ ] IO error swallowing replaced with `warn` logging or propagation
- [ ] `binary_name` validated to contain no path separators
- [ ] Low items addressed
- [ ] All tests pass, build and clippy clean
