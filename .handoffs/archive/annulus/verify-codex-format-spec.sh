...
check "codex format spec exists" \
  test -f "$ROOT/annulus/docs/providers/codex.md"

check "format section present" \
  rg -q '^## Format' "$ROOT/annulus/docs/providers/codex.md"

check "token-fields section present" \
  rg -q '^## Token fields' "$ROOT/annulus/docs/providers/codex.md"

check "session-boundary section present" \
  rg -q '^## Session boundary' "$ROOT/annulus/docs/providers/codex.md"

check "fixture directory exists" \
  test -d "$ROOT/annulus/tests/fixtures/codex"

check "fixture README documents provenance" \
  test -f "$ROOT/annulus/tests/fixtures/codex/README.md"

check "at least one fixture file present" \
  bash -c "ls '$ROOT/annulus/tests/fixtures/codex/' | grep -v '^README.md$' | head -1 | grep -q ."
