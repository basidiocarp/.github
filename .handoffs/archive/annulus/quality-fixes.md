...
- [x] TTL staleness check uses `saturating_add`
- [x] Model name match uses prefix or exact matching, not loose substring
- [x] `cast_sign_loss` addressed with explicit guard or `try_from`
- [x] `DegradationTier` wildcard replaced with explicit variant arms
- [x] All tests pass, clippy clean

## Verification Output (2026-04-21)

```
cargo build: Finished `dev` profile [unoptimized + debuginfo] target(s) in 0.66s
cargo test:  5 passed; 0 failed (+ 3 doctests ok)
cargo clippy: 4 pre-existing warnings (complex type, u64→u32 truncation x2, f64→f32 truncation)
              cast_sign_loss warning resolved
```
